...
function github(userRepo) {
  ...
  git()
    ...
    .then(() => {
      const { prompt, postScripts } = require(path.join(process.cwd(), TEMP_DIR, 'lz.config.js'))
      ...
      return Promise.all([inquirer.prompt(prompt), Promise.resolve(postScripts), filterBranch])
    })
    .then(([answers, postScripts]) => {
