...
const dynamicFileDestination = require('./dynamicFileDestination')

function generate(templatePath, answers, skip) {
  ...
  function createFileIn(filePath) {
    const fileName = dynamicFilename(filePath, answers)
    const destination = dynamicFileDestination(
      path.dirname(filePath.replace(templateDir + '/', '')),
      answers,
    )
